...
/*
 * $Id: dbmanager.php,v 1.3 2006/03/22 09:57:18 mikhail Exp $
...
function addColumn($post)
{
    ...
    if ($table == $mydirname) {
        if ('cat_id' == $column) {
            ...
            $sql = sprintf(
                'ALTER TABLE %s ADD trackbacks INT(11) NOT NULL default \'0\' ',
                $xoopsDB->prefix($mydirname)
            );
        } elseif ('permission_group' == $column) {
            $sql = sprintf(
                'ALTER TABLE %s ADD permission_group varchar(255) NOT NULL default \'all\' ',
                $xoopsDB->prefix($mydirname)
            );
        } elseif ('dobr' == $column) {
            $sql = sprintf(
                'ALTER TABLE %s ADD dobr tinyint(1) unsigned NOT NULL default \'1\' ',
                $xoopsDB->prefix($mydirname)
            );
        } else {
...
function addTable($post)
{
    ...
    if ($table == $mydirname . '_category') {
        ...
        $sql .= ') ENGINE = ISAM;';
    } elseif ($table == $mydirname . '_trackback') {
        ...
        $sql .= ') ENGINE = ISAM;';
    } elseif ($table == $mydirname . 'myalbum_photos') {
...
function checkTables()
{
    ...
    $columns = ['blog_id', 'tb_url', 'blog_name', 'title', 'description', 'link', 'direction', 'trackback_created'];
