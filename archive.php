<?php
/**
 * $Id: archive.php,v 1.3 2006/03/22 09:57:17 mikhail Exp $
...
// specify template
$GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_archive.html';
...
foreach ($cat_array as $cat) {
    $catselbox[$i]['prefix'] = mb_substr(str_replace('.', '--', $cat['prefix']), 2);

    $catselbox[$i]['cat_id'] = htmlspecialchars($cat['cat_id'], ENT_QUOTES | ENT_HTML5);

    $catselbox[$i]['cat_title'] = 0 == htmlspecialchars($cat['cat_id'], ENT_QUOTES | ENT_HTML5) ? '_BL_SELECT_ALLCATEGORY' : htmlspecialchars($cat['cat_title'], ENT_QUOTES | ENT_HTML5);

    $catselbox[$i]['selected'] = ($cat_id == $cat['cat_id']) ? ' selected' : '';

    $i++;

    $catbox[$cat['cat_id']] = htmlspecialchars($cat['cat_title'], ENT_QUOTES | ENT_HTML5);
}
...
$sql = sprintf(
    'SELECT user_id,uname FROM %s,%s WHERE uid=user_id AND private=\'N\' GROUP BY user_id ',
    $xoopsDB->prefix($mydirname),
    $xoopsDB->prefix('users')
);
if ($uid_result = $xoopsDB->query($sql)) {
    while (false !== ($uid_uname = $xoopsDB->fetchArray($uid_result))) {
 ...
 */
$sql = sprintf(
    'SELECT created FROM %s GROUP BY left(from_unixtime(created+%d)+0,6) ORDER BY created DESC',
    $xoopsDB->prefix($mydirname),
    $useroffset * 3600,
    '%Y%'
);
...
while (false !== ($created_example = $xoopsDB->fetchArray($month_result))) {
    [$this_year, $this_month] = explode(':', date('Y:m', $created_example['created']));
...
function makeTplVars()
{
    ...
    foreach ($results as $entryObject) {
        ...
        $entry['title'] = sprintf(
            '<a href=\'detail.php?blog_id=%d\'>%s</a>',
        ...
        $entry['cat_title'] = $catbox[$entryObject->getVar('cat_id')] ?? '';
