...
12 => _CAL_DECEMBER,
        ...
        while (false !== ($line = $xoopsDB->fetchArray($lines))) {
            //$months[$i]['date'] = $line['thismonth']."00";

            $months[$i]['year'] = (string)mb_substr($line['thismonth'], 0, 4);

            $months[$i]['month'] = (string)mb_substr($line['thismonth'], 4, 2);

            $months[$i]['monthname'] = $month_arr[(int)mb_substr($line['thismonth'], 4, 2)];
    ...
    function b_weblog_archive_edit($options)
    {
        ...
        $form .= sprintf(
            ...
            _MB_WEBLOG_EDIT_ARCHIVE_NUMBER_PER_PAGE,
            (int)$options[1]
        );
    ...
    function b_weblog_calendar_show($options)
    {
        ...
        require_once XOOPS_ROOT_PATH . '/modules/' . $mydirname . '/include/PEAR/Date/Calc.php';
        ...
        $date = (isset($_GET['date']) && mb_strlen((int)$_GET['date']) >= 6) ? (int)$_GET['date'] : '';

        if ($date > 0) {
            $month = mb_substr($date, 0, 6);
        } else {
        ...
        $this_year = mb_substr((string)$month, 0, 4);

        $this_month = mb_substr((string)$month, 4, 6);
...
12 => _CAL_DECEMBER,
        ...
        require_once XOOPS_ROOT_PATH . '/modules/' . $mydirname . '/class/class.calendar.php';
