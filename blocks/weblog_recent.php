<?php
/*
 * $Id: weblog_recent.php,v 1.3 2006/03/22 09:57:20 mikhail Exp $
...
if (!defined('WEBLOG_BLOCK_RECENT_INCLUDED')) {
    ...
    function b_weblog_top_weblogs_show($options)
    {
        ...
        $sql = sprintf(
            'SELECT b.user_id, b.created, u.uname, u.user_avatar, count(b.blog_id) AS count FROM %s AS b, %s AS u',
            $xoopsDB->prefix($mydirname),
            $xoopsDB->prefix('users')
        );

        $sql = sprintf(
            '%s WHERE (b.private = \'N\' OR b.user_id=\'%d\')  AND b.user_id=u.uid GROUP BY b.user_id ORDER BY count DESC, b.created DESC',
            $sql,
            $currentuid
        );
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            if (1 == $link_entries) {
                $entry['profile_uri'] = sprintf(
                    '%s/modules/%s/index.php?user_id=%d',
                    XOOPS_URL,
                    $mydirname,
                    $myrow['user_id']
                );
    ...
    function b_weblog_top_weblogs_edit($options)
    {
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_NUMBER_OF_ENTRIES,
            (int)$options[1]
        );

        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_USE_AVATARS,
            (int)$options[2]
        );

        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_LINK_TO_LIST,
            (int)$options[3]
        );
    ...
    function b_weblog_recent_show($options)
    {
        ...
        [$bl_contents_field, $permission_group_sql] = weblog_create_permissionsql($wb_configs, $mydirname);

        $sql = sprintf(
            'SELECT bl.blog_id, bl.created+%d AS created, bl.user_id, bl.title, %s AS contents, bl.comments, bl.`reads`, bl.trackbacks, u.uname, u.user_avatar FROM %s AS bl, %s AS u',
            $useroffset * 3600,
            $bl_contents_field,
            $xoopsDB->prefix($mydirname),
            $xoopsDB->prefix('users')
        ...
        $sql = sprintf(
            '%s WHERE ((bl.private = \'N\' OR bl.user_id=\'%d\') AND bl.user_id=u.uid %s ) ',
            $sql,
            $currentuid,
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            if ($show_contents && $max_size_contents) {
                if (!class_exists('WeblogEntryBase')) {
                    require_once XOOPS_ROOT_PATH . '/modules/' . $mydirname . '/class/entry.php';
                }
            ...
            $entry['uname'] = htmlspecialchars($myrow['uname'], ENT_QUOTES | ENT_HTML5);
            ...
            $entry['entry_url'] = sprintf(
                '%s/modules/%s/details.php?blog_id=%d',
                XOOPS_URL,
                $mydirname,
                $myrow['blog_id']
            );
    ...
    function b_weblog_recent_edit($options)
    {
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_MAX_TITLE_LENGTH,
            (int)$options[2]
        );

        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%s"></td></tr>',
            _MB_WEBLOG_EDIT_DATE_FORMAT,
            $options[3]
        );
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_SHOW_CONTENTS,
            (int)$options[7]
        );

        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_MAX_CONTENTS_LENGTH,
            (int)$options[8]
        );
    ...
    function b_weblog_users_weblogs_show($options)
    {
        ...
        if (0 == $order_by) {     // last entry time
            $user_sql = sprintf(
                'SELECT user_id,uname,user_avatar,max(created)+%d AS sort_value FROM %s AS bl , %s AS u WHERE bl.user_id=u.uid ',
                $useroffset * 3600,
                $xoopsDB->prefix($mydirname),
                $xoopsDB->prefix('users')
            );
            ...
            $user_sql = sprintf(
                'SELECT user_id,uname,user_avatar,sum(`reads`) AS sort_value FROM %s AS bl , %s AS u WHERE bl.user_id=u.uid ',
                $xoopsDB->prefix($mydirname),
                $xoopsDB->prefix('users')
            );
        ...
        while (false !== ($userrow = $xoopsDB->fetchArray($result_user))) {
            $user_id = htmlspecialchars($userrow['user_id'], ENT_QUOTES | ENT_HTML5);

            $user_sortvalue = (0 == $order_by) ? date($date_format, htmlspecialchars($userrow['sort_value'], ENT_QUOTES | ENT_HTML5)) : htmlspecialchars($userrow['sort_value'], ENT_QUOTES | ENT_HTML5);

            $users[$user_id] = [
                'uname' => htmlspecialchars($userrow['uname'], ENT_QUOTES | ENT_HTML5),
                'avatar_img' => sprintf('%s/uploads/%s', XOOPS_URL, htmlspecialchars($userrow['user_avatar'], ENT_QUOTES | ENT_HTML5)),
            ...
            while (false !== ($entryrow = $xoopsDB->fetchArray($result_entry))) {
                ...
                $users[$user_id]['entries'][] = [
                    'created' => date($date_format, htmlspecialchars($entryrow['created'], ENT_QUOTES | ENT_HTML5)),
                    'title' => xoops_substr(htmlspecialchars($entryrow['title'], ENT_QUOTES | ENT_HTML5), 0, $max_size),
                    'contents' => $contents,
                    'entry_url' => $entry_url,
                    'reads' => htmlspecialchars($entryrow['reads'], ENT_QUOTES | ENT_HTML5),
                    'comments' => htmlspecialchars($entryrow['comments'], ENT_QUOTES | ENT_HTML5),
                    'trackbacks' => htmlspecialchars($entryrow['trackbacks'], ENT_QUOTES | ENT_HTML5),
    ...
    function b_weblog_users_weblogs_edit($options)
    {
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_NUMBER_OF_USERS,
            (int)$options[1]
        );
    ...
    function b_weblog_recent_tb_show($options)
    {
        ...
        $sql = sprintf(
            ...
            $useroffset * 3600,
            $bl_contents_field,
            $xoopsDB->prefix($mydirname . '_trackback'),
            $xoopsDB->prefix($mydirname)
        );

        $sql = sprintf(
            '%s WHERE t.blog_id=bl.blog_id and (bl.private = \'N\' OR bl.user_id=\'%d\') and t.direction=\'recieved\' %s ',
            $sql,
            $currentuid,
            $permission_group_sql
        );
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            $tb_title = htmlspecialchars($myrow['tb_title'], ENT_QUOTES | ENT_HTML5);

            if (3 != $block_size) {
                if (mb_strlen($tb_title) >= $max_size) {
                    $tb_title = xoops_substr($tb_title, 0, ($max_size - 1));
                }
            }

            $trackback['blog_name'] = htmlspecialchars($myrow['blog_name'], ENT_QUOTES | ENT_HTML5);

            $trackback['link'] = htmlspecialchars($myrow['link'], ENT_QUOTES | ENT_HTML5);

            $trackback['tb_title'] = $tb_title;

            $trackback['entry_url'] = sprintf(
                '%s/modules/%s/details.php?blog_id=%d',
                XOOPS_URL,
                $mydirname,
                $myrow['blog_id']
            ...
            $trackback['entry_title'] = htmlspecialchars($myrow['entry_title'], ENT_QUOTES | ENT_HTML5);
    ...
    function b_weblog_recent_tb_edit($options)
    {
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_NUMBER_OF_TRACKBACKS,
            (int)$options[1]
        );
    ...
    function b_weblog_recent_comment_show($options)
    {
        ...
        $sql = sprintf(
            '%s WHERE com_modid=mid and dirname=\'%s\' and (bl.private = \'N\' OR bl.user_id=\'%d\') and blog_id=com_itemid and com_status=2 and (com_uid=uid or com_uid=0) %s group by com_id',
            $sql,
            $mydirname,
            $currentuid,
            $permission_group_sql
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            $com_title = htmlspecialchars($myrow['com_title'], ENT_QUOTES | ENT_HTML5);

            if (3 != $block_size) {
                if (mb_strlen($com_title) >= $max_size) {
            ...
            $comment['entry_title'] = htmlspecialchars($myrow['title'], ENT_QUOTES | ENT_HTML5);
            ...
            if (0 != $myrow['com_uid']) {
                $comment['profile_uri'] = sprintf('%s/userinfo.php?uid=%d', XOOPS_URL, htmlspecialchars($myrow['com_uid'], ENT_QUOTES | ENT_HTML5));
            }
    ...
    function b_weblog_recent_comment_edit($options)
    {
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%d"></td></tr>',
            _MB_WEBLOG_EDIT_NUMBER_OF_COMMENTS,
            (int)$options[1]
        );
    ...
    function b_weblog_recent_image_show($options)
    {
        ...
        if (function_exists('gd_info')) {
            ...
            if ('bundled (2' != mb_substr($gd_infomation['GD Version'], 0, 10)) {    // GD version 2 is required
                return false;
            }
        }
        ...
        $sql = sprintf(
            'SELECT blog_id, user_id , uname, created+%d AS created, title, contents , %s AS contents_p FROM %s AS bl , %s AS u',
            $useroffset * 3600,
            $bl_contents_field,
            $xoopsDB->prefix($mydirname),
            $xoopsDB->prefix('users')
        ...
        $sql = sprintf(
            '%s WHERE bl.user_id=u.uid and (bl.private = \'N\' OR bl.user_id=\'%d\') and bl.contents like \'%s\' %s ',
            $sql,
            $currentuid,
            '%' . $weblog_image_url . '%',
            $permission_group_sql
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            if ($max_contents_length > 0) {
                ...
                if (mb_strlen($contents) >= $max_contents_length) {
            ...
            $image['entry_url'] = sprintf(
                '%s/modules/%s/details.php?blog_id=%d',
                XOOPS_URL,
                $mydirname,
                htmlspecialchars($myrow['blog_id'], ENT_QUOTES | ENT_HTML5)
            );

            $image['blog_uri'] = sprintf(
                '%s/modules/%s/index.php?user_id=%d',
                XOOPS_URL,
                $mydirname,
                htmlspecialchars($myrow['user_id'], ENT_QUOTES | ENT_HTML5)
            );
    ...
    function b_weblog_recent_image_edit($options)
    {
        ...
        $form .= sprintf(
            '<tr><td><b>%s</b>:</td><td><input type="text" name="options[]" value="%s"></td></tr>',
            _MB_WEBLOG_EDIT_SHOW_FORBIDDEN_PICTURE,
            $options[5]
        );
    ...
    function b_weblog_category_list_show($options)
    {
        ...
        $cattree = new XoopsTree($xoopsDB->prefix($mydirname . '_category'), 'cat_id', 'cat_pid');
        ...
        foreach ($array as $cat) {
            ...
            $category['cat_title'] = htmlspecialchars($cat['cat_title'], ENT_QUOTES | ENT_HTML5);
            ...
            [$category['count']] = $xoopsDB->fetchRow($xoopsDB->query($count_sql));

            $prefix_num = (int)mb_strlen($cat['prefix']);
    ...
    function b_weblog_category_list_edit($options)
    {
        ...
        $sel1 = (1 == $options[1]) ? " selected='selected'" : '';

        $sel2 = (2 == $options[1]) ? " selected='selected'" : '';
    ...
    function get_wb_moduleconfig($mydirname)
    {
        ...
        $sql = sprintf(
            'SELECT conf_name,conf_value FROM %s,%s WHERE mid=conf_modid AND dirname=\'%s\' ',
        ...
        while (false !== ($rows = $xoopsDB->fetchArray($result))) {
