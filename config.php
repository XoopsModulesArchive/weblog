<?php
/**
 * $Id: config.php,v 1.4 2006/03/22 09:57:17 mikhail Exp $
...
if (!defined('_WEBLOG_COMMON_FUNCTIONS')) {
    ...
    function weblog_return_enc($data_string = false, $suppose_enc = false)
    {
        if (empty($suppose_enc)) {
            ...
            if (function_exists('mb_detect_encoding') && $data_string && $suppose_enc = mb_detect_encoding($data_string, mb_detect_order(), true)) {
                return $suppose_enc;
            }
    ...
    function weblog_create_permissionsql($blockModuleConfig = [], $mydirname = '')
    {
        ...
        if (empty($xoopsModule) || ('xoopsmodule' == get_class($xoopsModule) && $xoopsModule->dirname() != $mydirname)) {
        ...
        if ($config['use_permissionsystem'] && !$isAdmin) {
            if ($config['permission_showtitle']) {
                ...
                $permission_group_sql .= ' user_id=' . $currentuid . ')';
