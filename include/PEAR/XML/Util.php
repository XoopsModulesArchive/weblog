...
// +----------------------------------------------------------------------+
//
//    $Id: Util.php,v 1.1 2006/03/29 05:57:11 mikhail Exp $
...
'&quot;' => '"',
     ...
     * @param null       $encoding character encoding
     * @param null|mixed $standalone
     ...
     * @param null   $uri         uri of the doctype definition (or array with uri and public id)
     * @param null   $internalDtd internal dtd entries
    ...
    public function attributesToString($attributes, $sort = true, $multiline = false, $indent = '    ', $linebreak = "\n", $entities = XML_UTIL_ENTITIES_XML)
    {
        ...
        if (is_array($attributes) && !empty($attributes)) {
            ...
            if (!$multiline || 1 == count($attributes)) {
                foreach ($attributes as $key => $value) {
                    if (XML_UTIL_ENTITIES_NONE != $entities) {
                        if (XML_UTIL_CDATA_SECTION === $entities) {
                            $entities = XML_UTIL_ENTITIES_XML;
                        }
    ...
    {
        if (XML_UTIL_COLLAPSE_XHTML_ONLY == $mode) {
            return preg_replace(
     ...
     * @param mixed  $content
     * @param null   $namespaceUri    URI of the namespace
     ...
     */
    public function createTag($qname, $attributes = [], $content = null, $namespaceUri = null, $replaceEntities = XML_UTIL_REPLACE_ENTITIES, $multiline = false, $indent = '_auto', $linebreak = "\n")
    {
        $tag = [
            'qname' => $qname,
'attributes' => $attributes,
     ...
     */
    public function createStartElement($qname, $attributes = [], $namespaceUri = null, $multiline = false, $indent = '_auto', $linebreak = "\n")
    {
        // if no attributes hav been set, use empty attributes
        ...
        if (true === $multiline) {
            if ('_auto' === $indent) {
                $indent = str_repeat(' ', (mb_strlen($qname) + 2));
            }
        }
     ...
     * @param null   $defaultNs default namespace (optional)
