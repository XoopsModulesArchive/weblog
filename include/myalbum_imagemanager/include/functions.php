...
function myalbum_update_photo($lid, $cid, $title, $desc, $valid = null, $ext = '', $x = '', $y = '')
{
    ...
    if (isset($valid)) {
        ...
        if (1 == $valid) {
            ...
            $notificationHandler->triggerEvent('global', 0, 'new_photo', ['PHOTO_TITLE' => $title, 'PHOTO_URI' => "$mod_url/photo.php?lid=$lid&cid=$cid"]);
            ...
            $notificationHandler->triggerEvent('category', $cid, 'new_photo', ['PHOTO_TITLE' => $title, 'CATEGORY_TITLE' => $cat_title, 'PHOTO_URI' => "$mod_url/photo.php?lid=$lid&cid=$cid"]);
        }
    } else {
        $set_status = '';
    }
...
function myalbum_get_cat_options($order = 'title', $preset = 0, $prefix = '--', $none = null, $table_name_cat = null, $table_name_photos = null)
{
    ...
    while (list($title, $cid, $pid, $num) = $xoopsDB->fetchRow($rs)) {
        $cats[$key] = ['cid' => (int)$cid, 'pid' => (int)$pid, 'next_key' => $key + 1, 'depth' => 0, 'title' => htmlspecialchars($title, ENT_QUOTES | ENT_HTML5), 'num' => (int)$num];
