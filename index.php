<?php
/*
 * $Id: index.php,v 1.3 2006/03/22 09:57:17 mikhail Exp $
...
if (!$isAdmin && !checkprivilege('read_index', $xoopsModule->dirname(), $cat_id)) {
    redirect_header(
        sprintf('%s/index.php', XOOPS_URL),
        5,
        _BL_CANNOT_READ_SORRY
    );
...
// specify template
$GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_entries.html';
...
if (2 == $xoopsModuleConfig['update_reads_when'] && $user_id > 0
    || 3 == $xoopsModuleConfig['update_reads_when']) {
    $updateReads = true;
} else {
    $updateReads = false;
}
...
foreach ($weblog_result as $entryObject) {
    ...
    if (true === $updateReads) {
        ...
        if (empty($weblog_read[$blog_id]) || $curtime > $weblog_read[$blog_id] + $xoopsModuleConfig['expiration']) {
...
'date' => date("Y-m-d\TH:i:s", $entryObject->getVar('created')) . $TZ,
    ...
    $entry['created_date'] = formatTimestamp(
        $entryObject->getVar('created'),
        $xoopsModuleConfig['dateformat'],
        $xoopsConfig['default_TZ']
    );

    $entry['created_time'] = formatTimestamp(
        $entryObject->getVar('created'),
    ...
    $path = $weblogcat->getNicePathFromId(
    ...
    $entry['comlink'] = sprintf(
        '%s/modules/%s/details.php?blog_id=%d#comment',
        XOOPS_URL,
        $xoopsModule->dirname(),
    ...
    $entry['read_users_blog'] = sprintf(
        '<a href="index.php?user_id=%d">%s</a>',
    ...
    $entry['tracklink'] = sprintf(
        '%s/modules/%s/details.php?blog_id=%d#trackback',
        XOOPS_URL,
        $xoopsModule->dirname(),
        $entryObject->getVar('blog_id')
    );
...
$cat_url .= $weblogcat->getNicePathFromId(
    $cat_id,
    sprintf(
        '%s/modules/%s/index.php?user_id=%d',
        XOOPS_URL,
