<?php
/*
 * $Id: post.php,v 1.4 2006/03/22 09:57:17 mikhail Exp $
...
function &getEntry($post)
{
    ...
    if (isset($post['permission_group']) && is_array($post['permission_group'])) {
        ...
        $permission_group = $post['permission_group'] ?? '';
    }
...
if (!$isAdmin && (!$currentuid || !checkprivilege('edit', $xoopsModule->dirname()))) {
    redirect_header(
        sprintf('%s/modules/%s/index.php', XOOPS_URL, $xoopsModule->dirname()),
        5,
...
if (!empty($_POST['post'])) {
    ...
    if (mb_strlen($entry->getVar('contents', 'n')) < $xoopsModuleConfig['minentrysize']) {
...
'continue' => 1,
            ...
            sprintf(
                ...
                mb_strlen($entry->getVar('contents', 'n'))
            ),
...
'PAGE_URI' => $pageUri,
    ...
    if ($goodPost) {
        ...
        if ($entry->isUpdateping()) {
            ...
            $commonping_server_file = sprintf(
                '%s/modules/%s/language/%s/commonping_servers.inc.php',
                XOOPS_ROOT_PATH,
                $xoopsModule->dirname(),
...
'ok' => 1,
            ...
            'post.php',
            sprintf(_BL_CONFIRM_DELETE, stripslashes($entry->getVar('title'))),
            false,
            true
        );
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'weblog' . $mydirnumber . '_post.html';
    ...
    require_once sprintf('%s/modules/%s/include/myformdatetime.php', XOOPS_ROOT_PATH, $xoopsModule->dirname());

    require_once sprintf('%s/modules/%s/include/myformtextdateselect.php', XOOPS_ROOT_PATH, $xoopsModule->dirname());
    ...
    if (isset($xoopsModuleConfig['use_imagemanager']) && $xoopsModuleConfig['use_imagemanager']) {
        ...
        if ($imagemanager_valid) {
            $imagemanager_tag = sprintf(
                '<img onmouseover=\'style.cursor="hand"\' onclick=\'javascript:openWithSelfMain("%s/modules/%s/weblog-imagemanager.php?target=contents","imgmanager",400,430);\' src="%s/modules/%s/images/weblog_imagemanager.gif" alt="image">',
                XOOPS_URL,
                $xoopsModule->dirname(),
                XOOPS_URL,
                $xoopsModule->dirname()
    ...
    $trackback_textarea = new XoopsFormTextArea(
        _BL_TRACKBACK_DSC . '<br>',
        'ent_trackbackurl',
    ...
    if (isset($recieved_trackback) && is_array($recieved_trackback) && count($recieved_trackback) > 0) {
        ...
        foreach ($recieved_trackback as $key => $tb_obj) {
            ...
            $recieved_trackback->addOption($tb_obj->getVar('tb_url'), htmlspecialchars(xoops_substr($tb_obj->getVar('title', 'n') . ':' . $tb_obj->getVar('description', 'n'), 0, 60) . '...', ENT_QUOTES | ENT_HTML5));
