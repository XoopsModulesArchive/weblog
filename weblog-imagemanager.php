...
if (count($cats) > 0) {
    ...
    foreach ($cats as $cat) {
        $newcat = htmlspecialchars($cat['cat_title'], ENT_QUOTES | ENT_HTML5);

        $prefix = str_replace('.', '--', mb_substr($cat['prefix'], 1));

        $photo_count = $photo_counts[$cat['cat_id']] ?? 0;

        if ($cid == $cat['cat_id']) {
            $cat_options .= "<option value='{$cat['cat_id']}' selected='selected'>$prefix{$newcat} ($photo_count)</option>\n";
        } else {
            ...
            while (list($lid, $cid, $title, $ext, $submitter, $res_x, $res_y, $is_normal) = $xoopsDB->fetchRow($prs)) {
                ...
                $xoopsTpl->append(
                    ...
                    [
                        ...
                        'res_x' => $res_x,
                        'res_y' => $res_y,
                        'nicename' => htmlspecialchars($title, ENT_QUOTES | ENT_HTML5),
