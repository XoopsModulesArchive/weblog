<?php
/*
 * $Id: weblog-tb.php,v 1.1 2006/03/29 05:57:05 mikhail Exp $
...
// get trackback ID
if (isset($_SERVER['SERVER_SOFTWARE']) && false !== stripos($_SERVER['SERVER_SOFTWARE'], "Microsoft-IIS")) {
...
if (function_exists('mb_detect_encoding')) {
    $data['encoding'] = mb_detect_encoding($data['description'], mb_detect_order(), true);
} else {
    $data['encoding'] = 'auto';
}
...
function return_trackback_data($r_trackback_array)
{
    ...
    foreach ($r_trackback_array as $trackback_obj) {
        if ('weblogtrackbackbase' != mb_strtolower(get_parent_class($trackback_obj))) {
            continue;
        }
